...
2. 빌드합니다.
   ...
   ```bash
   node ~/.claude/skills/design-deck/build.js hello.md
   open hello.html
   ```

끝. 우측 상단에 오렌지임팩트 로고, Pretendard 폰트, 오렌지 강조색이 자동으로 들어갑니다. `Cmd+P` → Save as PDF 로 슬라이드당 1페이지 PDF가 나옵니다. 또는 `--pdf` 플래그로 자동 생성.
