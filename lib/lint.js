...
// im-not-ai (epoko77-ai) S1 patterns — high-confidence AI-tells in Korean.
// Detection only; the build does not auto-rewrite. Authors should fix in
// place using the rules from prompts/slide-authoring-rules.md.
const AI_TELL_D_RE = /결론적으로|시사하는 바|본 발표에서는|본 자료는|주목할 만하다|혁신적인|획기적인|전례 없는/;
const AI_TELL_A_PASSIVE_RE = /(?:되어지|보여지|불려지|쓰여지)[다는]/;
const AI_TELL_C_MECH_RE = /(?:첫째|첫번째|첫 번째)[\s,.][\s\S]{0,200}(?:둘째|두번째|두 번째)[\s,.][\s\S]{0,200}(?:셋째|세번째|세 번째)/;
const AI_TELL_I_FORMAL_RE = /것이다|점이다|바이다|할 필요가 있다/g;
...
export function lintDeck(slides, opts = {}) {
  ...
  for (const s of slides) {
    ...
    walk(s.tokens, (t) => {
      ...
      // im-not-ai S1 patterns
      if (text && AI_TELL_D_RE.test(text)) {
        const m = text.match(AI_TELL_D_RE);
        warnings.push({ slide: s.index + 1, rule: "ai-tell-d", text: m ? m[0] : text.slice(0, 40) });
      }
      if (text && AI_TELL_A_PASSIVE_RE.test(text)) {
        const m = text.match(AI_TELL_A_PASSIVE_RE);
        warnings.push({ slide: s.index + 1, rule: "ai-tell-a-passive", text: m ? m[0] : text.slice(0, 40) });
      }
      if (text && AI_TELL_C_MECH_RE.test(text)) {
        warnings.push({ slide: s.index + 1, rule: "ai-tell-c-mech", text: "첫째/둘째/셋째 동시 등장" });
      }
      if (text) {
        const formal = text.match(AI_TELL_I_FORMAL_RE);
        if (formal && formal.length >= 3) {
          warnings.push({ slide: s.index + 1, rule: "ai-tell-i-formal", text: `형식명사 ${formal.length}회` });
        }
      }
