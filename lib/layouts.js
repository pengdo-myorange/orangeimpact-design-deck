...
function wrap(slide, ctx, inner, extraClasses = "") {
  ...
  // `data-part` lets the storyboard overlay group slides into part-blocks
  // and the part-level "approve part" button. parse.js stamps this onto
  // every slide based on `# Part N:` headings or explicit `<!-- part: N -->`
  // directives.
  const partAttr = slide.part ? ` data-part="${slide.part}"` : "";
  return (
    `<section class="${cls.filter(Boolean).join(" ")}" data-screen-label="${escapeHTML(label)}"${themeAttr}${accentAttr}${partAttr}>` +
    logoHtml +
    chrome +
    inner +
    `</section>`
...
function renderPartCover(slide, ctx) {
  // Image-only by user requirement (orange-blog-img integration). The cover
  // slide renders as a single full-bleed gpt-image-2 image with no text or
  // shapes layered on top — the image carries the entire message. If no
  // image was supplied or generated, fall back to a flat brand-color panel
  // so the deck does not break.
  const img = slide.tokens.find((t) => t.type === "image");
  const src = img && (img.dataUrl || img.url);
  if (src) {
    const inner = `<div class="sl-part-image" style="background-image:url('${src}');"></div>`;
    return wrap(slide, ctx, inner, "sl-part-image-only");
  }
  // Fallback: solid brand color (no text — the cover is image-only by design).
  return wrap(slide, ctx, `<div class="sl-part-image sl-part-image-empty"></div>`, "sl-part-image-only");
}

function renderInfographicPending(slide, ctx) {
  // Placeholder shown until the user drops a NotebookLM PNG into
  // ./infographics/slide-N.png. The next build picks it up and swaps the
  // layout to `image` automatically (notebooklm-bridge.js). Until then we
  // keep the slide informative so reviewers know what's pending.
  const [h1, rest] = pickHeading(slide.tokens, 1);
  const title = h1 ? `<h1 class="sl-title">${inline(h1.text)}</h1>` : "";
  const body = renderTokensGeneric(rest, ctx);
  const expected = `infographics/slide-${slide.index + 1}.png`;
  const hint = `<div class="sl-infographic-pending"><strong>NotebookLM 인포그래픽 대기 중</strong><span>저장 경로: <code>${escapeHTML(expected)}</code></span><span>빌드 로그의 프롬프트를 NotebookLM Visual Overview 에 붙여넣고 PNG 저장 후 재빌드</span></div>`;
  return wrap(slide, ctx, `<div class="sl-body">${title}${body}${hint}</div>`);
}

function renderConceptImage(slide, ctx) {
  // Two-column body for abstract / metaphor slides: heading + body on the
  // left, gpt-image-2 generated illustration on the right. When the slide
  // sets `image_full: true` in frontmatter, the image fills the slide.
  const [h1, rest] = pickHeading(slide.tokens, 1);
  const img = (slide.tokens.find((t) => t.type === "image") || rest.find((t) => t.type === "image"));
  const src = img && (img.dataUrl || img.url);
  const fullBleed = slide.directives && (slide.directives["image-full"] === "true" || slide.directives["image_full"] === "true");
  if (fullBleed && src) {
    return wrap(slide, ctx, `<div class="sl-concept-full" style="background-image:url('${src}');"></div>`, "sl-concept-image-full");
  }
  const title = h1 ? `<h1 class="sl-title sl-concept-title">${inline(h1.text)}</h1>` : "";
  const bodyTokens = rest.filter((t) => t.type !== "image");
  const text = `<div class="sl-concept-text">${title}${renderTokensGeneric(bodyTokens, ctx)}</div>`;
  const figure = src
    ? `<div class="sl-concept-figure" style="background-image:url('${src}');"></div>`
    : `<div class="sl-concept-figure sl-concept-figure-empty"></div>`;
  return wrap(slide, ctx, `<div class="sl-body sl-concept-body">${text}${figure}</div>`);
}
...
function renderChart(slide, ctx) {
  ...
  if (dataFence) {
    for (const line of dataFence.content.split("\n")) {
      ...
      if (parts.length >= 2 && parts[0]) {
        ...
        // valueRaw keeps the raw value-column string so sparkline can
        // split "120 135 142 …" into a series without losing precision.
        items.push({ label: parts[0], value: parseFloat(parts[1]) || 0, valueRaw: parts[1] || "", value2: v2num, color });
      }
    }
  }
  ...
  // Tufte: bar charts default to gray (non-emphasis); donut/pie keep the
  // palette rotation so each slice is readable. Explicit color in the data
  // fence always wins; highlight directive promotes one row to brand.
  const grayFirst = kind === "bar";
  let enriched = items.map((it, i) => {
    const isHl = i + 1 === highlightIdx;
    const col = it.color
      ? resolveColor(it.color)
      : isHl
        ? "var(--brand)"
        : grayFirst
          ? "var(--neutral-400)"
  ...
  // Tufte: donut/pie segments follow meaningful sequence.
  //   • default = source order (preserves narrative)
  //   • <!-- order: magnitude --> = largest → smallest, clockwise from 12
  //   • "기타/other/misc/etc" rows are always pinned to the end
  if (kind === "donut" || kind === "pie") {
    const orderMode = String(slide.directives.order || "source").toLowerCase();
    const OTHER_RE = /^(기타|others?|misc|etc\.?|rest)$/i;
    const mainLabel = (it) => String(it.label || "").split(/\s+\/\s+/)[0].trim();
    const isOther = (it) => OTHER_RE.test(mainLabel(it));
    const others = enriched.filter(isOther);
    const main = enriched.filter((it) => !isOther(it));
    if (orderMode === "magnitude") main.sort((a, b) => b.value - a.value);
    enriched = [...main, ...others];
  }
  ...
  if (kind === "donut") {
    ...
    const slices = enriched.map((it) => {
      ...
      angle = endAngle;
      // Tufte: thin canvas-colored separator (theme-aware) instead of hard white.
      return `<path d="${path}" fill="${it.color}" stroke="var(--bg-canvas)" stroke-width="1"/><text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" font-size="22" font-weight="700" fill="white">${pct}%</text>`;
    }).join("");
    ...
    let x = 0;
    // Tufte: no rounded corners — the data-ink is the rectangle's length.
    const segs = enriched.map((it, i) => {
      const segW = (it.value / total) * segTotalW;
      const seg = `<g transform="translate(${x} 0)">
        <rect x="0" y="0" width="${segW}" height="${h}" fill="${it.color}"/>
    ...
    // Tufte's signature — word-sized line chart. One row of :::data = one series.
    // Value column is whitespace-separated numbers. Only last point gets a
    // filled marker + label; middle points stay implicit to keep the data-ink
    // ratio maximal.
    const series = enriched
      .map((it, idx) => {
        const nums = String(it.valueRaw || "")
          .split(/\s+/)
          .map((s) => parseFloat(s))
          .filter((n) => !isNaN(n));
        const col = it.color && it.color !== ("var(--neutral-400)") ? it.color : (idx === 0 ? "var(--brand)" : palette[idx % palette.length]);
        return { ...it, nums, color: col };
      })
      .filter((l) => l.nums.length > 1);
    if (!series.length) {
      chart = "";
      showLegend = false;
    } else {
      const w = 1200, h = 180, padX = 24, padTop = 24, padBot = 40;
      const allVals = series.flatMap((l) => l.nums);
      const yMin = Math.min(...allVals);
      const yMax = Math.max(...allVals);
      const yRange = yMax - yMin || 1;
      const maxLen = Math.max(...series.map((l) => l.nums.length));
      const stepX = (w - padX * 2) / Math.max(maxLen - 1, 1);
      const yAt = (v) => h - padBot - ((v - yMin) / yRange) * (h - padTop - padBot);
      const paths = series
        .map((l) => {
          const pts = l.nums.map((v, i) => [padX + i * stepX, yAt(v)]);
          const d = pts.map((p, i) => `${i === 0 ? "M" : "L"} ${p[0].toFixed(1)} ${p[1].toFixed(1)}`).join(" ");
          const [lx, ly] = pts[pts.length - 1];
          const last = l.nums[l.nums.length - 1];
          return (
            `<path d="${d}" fill="none" stroke="${l.color}" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>` +
            `<circle cx="${lx.toFixed(1)}" cy="${ly.toFixed(1)}" r="5" fill="${l.color}"/>` +
            `<text x="${(lx + 12).toFixed(1)}" y="${(ly + 6).toFixed(1)}" font-size="22" font-weight="700" fill="var(--fg-strong)" style="font-feature-settings:'tnum' 1;">${escapeHTML(String(last))}</text>`
          );
        })
        .join("");
      chart = `<svg class="sl-chart-svg sl-chart-sparkline" viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:${w}px;height:auto;">${paths}</svg>`;
      // Direct-labeled last point; legend only when there's more than one series.
      showLegend = series.length > 1;
    }
  } else {
    // bar — Tufte defaults: no rounded corners, gray baseline when no color
    // is given and no highlight directive is set, direct value labels at the
    // right end of each bar (so the legend value column is redundant).
    const max = Math.max(...enriched.map((it) => it.value), 1);
    const barH = 36, gap = 16, w = 520, labelPad = 96;
    const totalW = w + labelPad;
    const bars = enriched
      .map((it, i) => {
        const y = i * (barH + gap);
        const bw = (it.value / max) * w;
        const labelX = Math.max(bw + 12, 12);
        return (
          `<g transform="translate(0 ${y})">` +
          `<rect x="0" y="0" width="${bw}" height="${barH}" fill="${it.color}"/>` +
          `<text x="${labelX}" y="${barH / 2 + 2}" dominant-baseline="middle" font-size="22" font-weight="700" fill="var(--fg-strong)" style="font-feature-settings:'tnum' 1;">${escapeHTML(String(it.value))}</text>` +
          `</g>`
        );
      })
      .join("");
    const hSvg = enriched.length * (barH + gap);
    chart = `<svg class="sl-chart-svg" viewBox="0 0 ${totalW} ${hSvg}" width="${totalW}" height="${hSvg}" preserveAspectRatio="xMidYMid meet" style="width:100%;max-width:${totalW}px;height:auto;">${bars}</svg>`;
    // Direct-labeled; the value column in the legend would just repeat the
    // bar ends. Keep the legend for label↔color mapping only.
  }
...
function renderAxisDiagram(slide, ctx) {
  ...
  const dash = "stroke-dasharray='6 8' stroke='var(--neutral-200)'";
...
const LAYOUTS = {
  ...
  quote: renderQuote,
  image: renderImage,
  "concept-image": renderConceptImage,
  infographic: renderInfographicPending,
...
/* ============================================================
   Typography polish — text-wrap: pretty avoids orphan words
   at the end of headlines/captions. Supported in Chrome 114+,
   Safari 17.4+, Firefox 130+; falls back cleanly when absent.
   ============================================================ */
.sl-title,
.sl-body p,
.sl-body li,
...
/* ============================================================
   Part Cover
   ============================================================ */
/* Part cover — image-only (orange-blog-img integration).
   No text or shape layers are rendered on top of the image. The image fills
   the entire slide. If no image is available, the slide falls back to the
   solid brand color so the deck still prints cleanly. */
.sl-part-cover { background: #000000; }
.sl-part-cover.sl-part-image-only .sl-logo { display: none; }
.sl-part-image { position: absolute; inset: 0; background-size: cover; background-position: center; background-repeat: no-repeat; }
.sl-part-image-empty { background: var(--brand); }

/* Concept image — half-slide illustration for abstract/metaphor bodies. */
.sl-concept-body { display: grid; grid-template-columns: 1fr 1fr; gap: 80px; padding: 120px 160px; align-items: center; }
.sl-concept-text h1.sl-concept-title { font-size: var(--deck-heading-lg); line-height: 1.1; margin: 0 0 32px; letter-spacing: -1px; }
.sl-concept-text p { font-size: 24px; line-height: 1.55; color: var(--fg-alternative); margin: 0 0 16px; max-width: 720px; }
.sl-concept-figure { width: 100%; aspect-ratio: 1 / 1; border-radius: var(--radius-lg); background: #000000; background-size: contain; background-position: center; background-repeat: no-repeat; box-shadow: var(--shadow-md); }
.sl-concept-figure-empty { background: var(--bg-muted); }
.sl-concept-image-full .sl-concept-full { position: absolute; inset: 0; background-size: cover; background-position: center; background-repeat: no-repeat; }

/* Infographic pending — visible cue for slides awaiting a NotebookLM PNG. */
.sl-infographic-pending {
  margin-top: 40px;
  padding: 32px;
  ...
  font-size: 18px; color: var(--fg-strong);
}
.sl-infographic-pending strong { color: var(--brand); font-size: 22px; }
.sl-infographic-pending code { background: rgba(0,0,0,0.06); padding: 2px 8px; border-radius: 4px; font-size: 16px; }
...
.sl-statement-hero { font-size: 112px; line-height: 1.32; font-weight: 800; color: var(--fg-strong); margin: 0; letter-spacing: -2.4px; max-width: 1600px; }
