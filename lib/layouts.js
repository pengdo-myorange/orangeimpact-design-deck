...
function renderChart(slide, ctx) {
  ...
  const enriched = items.map((it, i) => {
    ...
    const col = it.color || (isHl ? "var(--brand)" : palette[i % palette.length]);
  ...
  const legend = showLegend ? enriched
    .map(
      (it) =>
        `<li class="sl-chart-legend-item"><span class="sl-chart-swatch" style="background:${it.color}"></span><span class="sl-chart-label">${escapeHTML(it.label)}</span><span class="sl-chart-value">${escapeHTML(String(it.value))}${it.value2 !== undefined ? ` / ${it.value2}` : ""}</span></li>`
    )
    .join("") : "";
  const title = h1 ? `<h1 class="sl-title">${inline(h1.text)}</h1>` : "";
...
.sl-bento-card { position: relative; background: var(--bg-canvas); border: 1px solid var(--border-subtle); border-radius: 24px; padding: 40px; display: flex; flex-direction: column; gap: 16px; }
