// Inline markdown: bold, italic, code, link, br.
// Escapes HTML first, then applies inline transforms in order.

const ESC = { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" };

export function escapeHTML(s) {
  return String(s).replace(/[&<>"']/g, (c) => ESC[c]);
}

// Mask out fenced spans so we don't re-process their contents.
function inline(raw) {
  if (!raw) return "";
  let s = escapeHTML(raw);

  // Inline code: `code`
  const codeStash = [];
  s = s.replace(/`([^`\n]+?)`/g, (_, c) => {
    codeStash.push(c);
    return `\x00CODE${codeStash.length - 1}\x00`;
  });

  // Links: [text](url)
  s = s.replace(/\[([^\]]+?)\]\(([^)\s]+?)(?:\s+&quot;([^&]*?)&quot;)?\)/g, (_, text, url, title) => {
    const t = title ? ` title="${title}"` : "";
    return `<a href="${url}"${t}>${text}</a>`;
  });

  // Bold: **text** or __text__ (bold takes precedence over italic)
  s = s.replace(/\*\*([^*\n]+?)\*\*/g, "<strong>$1</strong>");
  s = s.replace(/__([^_\n]+?)__/g, "<strong>$1</strong>");

  // Brand highlight: ==text== — bold in accent orange. Used to call out
  // 1-2 words per slide in the way the sample deck does
  // ("관찰자", "3.5배 증가", "AI의 출발선", etc.).
  s = s.replace(/==([^=\n]+?)==/g, '<mark class="sl-hi">$1</mark>');

  // Italic: *text* or _text_
  s = s.replace(/(^|[\s(])\*([^*\n]+?)\*/g, "$1<em>$2</em>");
  s = s.replace(/(^|[\s(])_([^_\n]+?)_/g, "$1<em>$2</em>");

  // Chip: [label]{.chip-orange|blue|gray} — renders as inline pill.
  s = s.replace(/\[([^\]\n]+?)\]\{\.chip-(orange|blue|gray)\}/g, (_, label, color) =>
    `<span class="sl-chip sl-chip-${color}">${label}</span>`
  );

  // Hard break: two trailing spaces + newline, or explicit <br>
  s = s.replace(/  \n/g, "<br>\n");
  // Allow literal <br> tags (single safe-listed HTML tag) to pass through.
  s = s.replace(/&lt;br\s*\/?&gt;/gi, "<br>");

  // Restore code spans
  s = s.replace(/\x00CODE(\d+)\x00/g, (_, i) => `<code>${codeStash[+i]}</code>`);

  return s;
}

// Exported as both a named fn and a default helper for convenience.
export default inline;
export { inline };
