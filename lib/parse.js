...
// Detect `# Part N: ...` (or `Part N` directive) and stamp every following
// slide with `part: N` until the next part marker. Used by the storyboard
// grid and the bulk part-approval UI. Part numbers are stable: a part-cover
// slide that names "Part 02" wins over a sequential counter.
function assignParts(slides) {
  let current = 0;
  for (const s of slides) {
    let detected = null;
    const h1 = (s.tokens || []).find((t) => t.type === "heading" && t.level === 1);
    if (h1) {
      const m = String(h1.text || "").match(/^\s*Part\s+0*(\d+)\b/i);
      if (m) detected = parseInt(m[1], 10);
    }
    if (s.directives && s.directives.part) {
      const n = parseInt(s.directives.part, 10);
      if (!Number.isNaN(n)) detected = n;
    }
    if (s.layout === "part-cover" && detected == null && h1) {
      // Sequential fallback for part-cover without explicit number.
      detected = current + 1;
    }
    if (detected != null) current = detected;
    s.part = current || 0;
  }
}
