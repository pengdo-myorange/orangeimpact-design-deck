...
export async function inlineImages(slides, opts) {
  const { mdDir, mode, cacheDir, openaiKey, noImageGen, focalColor } = opts;
  const stats = { generated: 0, hits: 0, placeholders: 0, costEstimate: 0 };
  // Each token carries the *slide layout* tag so image-gen.js can pick the
  // correct STYLE_RULES (cover → orange-blog-img full-bleed brand rules;
  // concept → half-slide negative-space rules; default → photographic).
  const all = [];
  const walk = (tokens, slideLayout) => {
    for (const t of tokens) {
      if (t && t.type === "image") all.push({ t, slideLayout });
      if (t && t.tokens) walk(t.tokens, slideLayout);
    }
  };
  for (const s of slides) walk(s.tokens, s.layout);

  for (const { t, slideLayout } of all) {
    if (t.isAI) {
      ...
      const kind = slideLayout === "part-cover" ? "cover"
        : slideLayout === "concept-image" ? "concept"
        : "default";
      try {
